<?php
	...
	function insertRowIntoUserDb($id, $password, $name, $sex, $dateBirth, $phoneNumber, $identityVerificationEmail){
		$con = dbConnection();
		$sql = "
			INSERT INTO user
	...
	function insertRowIntoReservationDb($id, $medicalDepartment, $medicalProfessor, $dateOfTreatment, $timeOfTreatment, $reservationDate, $symptomsEtc){
		$con = dbConnection();
		$sql = "
		INSERT INTO reservation
		(id, medicalDepartment, medicalProfessor, dateOfTreatment, timeOfTreatment, reservationDate, symptomsEtc)
		VALUES('{$id}', '{$medicalDepartment}', '{$medicalProfessor}', '{$dateOfTreatment}', '{$timeOfTreatment}', '{$reservationDate}', '{$symptomsEtc}'
		)";
		$result = mysqli_query($con, $sql);
		if($result == false){
		}
